...
def _try_append_omml(paragraph, latex: str, *, display: bool) -> bool:
    ...
    本包按设计不带 ``math_to_omml.py``（``requirements-oa.txt`` 无 ``latex2mathml``），
    公式按原文写入 Word；仅提示一次，不逐条刷屏。
    """
...
def _warn_bare_paren_latex(md_text: str) -> None:
    ...
    本包随带 ``latex_delimiters.py`` 副本。意见陈述一般无公式，命中时只提示
    ``LATEX_DELIM:``、不阻断转换（陈述书可能引用权要原文中的括号）。
    """
    try:
