...
- 输出策略与发明相同：**默认一篇**最有价值的交底；保护型 1+N 等首篇定稿后走 `prompts/fence/`；用户要多篇且未走该旁路时先列大纲再约定顺序。  
- 选定依据：构造可区分性、可实施、保护范围（装置书式）、查新后差异化、**图证链是否闭合**。查新走 `../prior_art_search.md`：结构区别能指到具体文献即可 **带疑点过**；指不到则 **不过** 仍成文、降级写，禁止假 D1。
