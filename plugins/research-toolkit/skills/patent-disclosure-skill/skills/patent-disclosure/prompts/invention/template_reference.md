...
- 结尾总结：过 / 带疑点过时，检索总结后再**点名最接近的一篇（D1）**——公开号 + 标题 + **一句选定理由**，随后给**以 D1 为主的逐特征比对表**。规则见 **`disclosure_builder.md` §7.10** 与 **`prior_art_search.md`「D1 锁定与区别特征 Fk」**。**不过**时不点名 D1，写明未钉住对比文件；区别对材料/常规做法列表，标明未经对比文件钉死。**禁止**只写「与现有技术存在本质区别」、**禁止**五篇并列逐特征、**禁止**把不相干命中标成 D1。门禁为带疑点过时，选定理由后加一句证据边界。

  ```markdown
  **最接近的现有技术（D1）**：CN1xxxxxxxxA《一种基于负载的任务调度方法》——与本案同为派发前二次排序，模块重叠最多，故取为 D1。（依据公布摘要，公开文本未通读。）
  ...
  **特征一览**（后文只用这些简称，F 号最多句末括注）：
  ...
  上表按本案替换；无组合风险则不要 D2 行。**禁止**「是否区别」列只写「是（F1）」；**禁止**列名叫「本专利权要」。Fk 在正文中的出场见 **`disclosure_builder.md` §7.10**（三 / 六不用 Fk 当主语）。

### 1.2 现有技术存在的缺点

- 分点列举，与 1.1 的局限性呼应；过 / 带疑点过时每条须能指到 **D1 或 D2** 的具体不足；**不过**时指到材料所述现有做法或本领域常规的具体不足
- 写完整缺点句，句末可括注（F1）；**禁止**「对应 F1：」当条目标题
- 不要写「现有技术普遍存在不足」，不要空喊「创新性强」
- 突出**核心缺陷**：该现有做法（或该对比文献）无法解决、而本案要解决的问题

---

## 二、本发明所要解决的技术问题

- 对应一中的缺点逐条写；每条由 Fk 的技术效果反推得到（三段绑定见 **`disclosure_builder.md` §7.10**），**正文只写问题句**，不要「（对应 F1）」
- **硬项**：技术问题只说「要达到什么」，**不得**写入本发明的手段词，也不得暗示手段。写「如何在不引起调度震荡的前提下降低长尾任务等待时长」，**不要**写「如何通过资源画像与限频重排队来……」
- 解决思路、模块与步骤放**第三章**；本章不铺方案
- 简明扼要，为第三章详细方案做铺垫
...
- 先概括性观点，再分点详述
- 每条优点写**完整效果句**，把带来该效果的构造/机制写进句子；句末可括注（F1）。**禁止**「由 F1 带来：」当句首；**禁止**只写「提升效率」「增强鲁棒性」而不写来源构造
- 存在协同关系时用一段整体论述（单独任一构造都达不到该组合效果），不要写成 F 号运算
...
**分 5.1 / 5.2 两小节**，按**删除测试**归类（删掉该特征后方案还能否解决第二章的技术问题：不能→必要，仍能→优选）。规则见 **`disclosure_builder.md` §7.10**。

```markdown
### 5.1 独立保护构思（必要技术特征）

- **F1** 对节点多维资源指标做窗口平滑聚合，得到资源画像（对应 3.3 模块 M1、式 (1)）
- **F2** 以间隔阈值与分数变化阈值双条件触发重排（对应 3.4 步骤 S3、式 (2)）
- **F3** 重排仅作用于队列窗口子集（对应 3.4 步骤 S4）

### 5.2 优选特征

- 窗口长度、阈值取值等可调参数（取值见 3.5，不作为必要特征）
- 心跳假死检测触发迁移（独立子主题，删除后主问题仍可解）
```

- 5.1 每条写「特征描述 + 对应 3.x / Sx / Mx / 式 (n)」，编号与 1.1 比对表的 **Fk 相同**；条目数宜等于站得住的 Fk 数。**每条必须复述特征全文**，离开 F 号仍可读。
- 每条仍须是**可实施机制**（对什么输入、如何判定或控制、得到什么输出），详细技术方案引用第三章。
- **禁止**写成正式权项句式（「根据权利要求 1…」）——交底书不是权利要求书；书式取向留给 `## 交付后请确认` 第 2 条。
- **禁止**把保护点写成仅有角色枚举（如「有三个数字员工」）或只复述标题对象名称。
- 只剩参数取值差异的 Fk 放 5.2，**不要**硬写成独立创新点。
- 若有 **≥2 组无共同必要特征**的发明点，在 5.1 写明**总的发明构思**或建议拆案。
- 避免与第三章重复大段技术细节。
