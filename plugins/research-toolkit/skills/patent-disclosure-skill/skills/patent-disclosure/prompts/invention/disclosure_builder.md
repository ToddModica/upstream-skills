...
3. 二、针对上述缺点，说明本发明所要解决的技术问题（由 **Fk 的技术效果反推**，**不得**写入本发明手段，见 **§7.10**）
4. 三、本发明技术方案的详细阐述
...
5. 四、与现有技术相比，本发明具有哪些优点？（每条挂到具体 Fk；有协同须整体论述，见 **§7.10**）
...
Step 5 已按 `prior_art_search.md`「D1 锁定与区别特征 Fk」落下 **`查新与区别定位_*.md`**（含门禁三态与证据级别）。本节把这条主线写进第一、二、四、五章：**Fk 编号全案唯一，四章复用同一编号、同一口径**，禁止各章各起一套说法。

内部定位稿用 D1/Fk 编号；交底正文要让代理人能跟上审查框架，**不必**把「唯一 D1」写成仪式性套话。过 / 带疑点过：点名最接近的一篇、给出选定理由、用技术维度对比即可，标签写「最接近的现有技术（D1）」。**不过**：见本节「降级成文」，**禁止**假 D1。

### 第一章：主比对钉一篇，允许 D2 补行

- **过 / 带疑点过**：1.1 在按技术方向列举之后，**点名最接近的一篇**：公开号 + 标题 + **一句选定理由**，随后给**逐特征比对表**（3～5 行，维度为技术维度）。其余检索条目作技术背景，**不要**五篇并列逐特征。
- **D2 补行**：仅当 D1 未覆盖、本案有、且 D2 可能被拿来组合时，在主表下加行并标明 D2；不是第二套出发点。
- 门禁为 **带疑点过** 时：选定理由后加**一句**证据边界（如「依据公布摘要，公开文本未通读」或「两篇接近，本篇作出发点、某某为 D2」），**禁止**写成已核实全文对比。
- 过 / 带疑点过：1.2 每条缺点须能落到 **D1 或 D2 一侧的具体不足**上。**禁止**只写「现有技术普遍存在不足」。
...
- 写明「本次检索未钉住可作最接近现有技术的对比文件」。区别对**材料里的现有做法 / 本领域常规**写，Fk 仍编号并贯穿后文，标明未经对比文件钉死。
- 1.2 指到上述现有做法的具体不足，**禁止**「本质区别」「创新性强」等无指向套话。
- 该拆案时 5.1 写明无共同必要特征，不要为了凑一条 D1 把两组发明点捏成假对比。

### 第二章：三段绑定（技术问题由技术效果反推）

每条站得住的 Fk 都要能填出三段：**Fk → 该特征在本方案中的技术效果 → 实际解决的技术问题**。第二章正文写的是**第三段**。
...
多条 Fk 在**功能上相互支持、共同**产生一个效果时（如 F1 给出分数、F2 用分数变化做触发），**按整体写一条三段**，不要拆成互不相干的三条——拆开后每条单独看都容易显得显而易见。

### 第四章：优点与 Fk 一一对应

- 每条优点须**标明由哪条 Fk 带来**；**禁止**只写「提升效率」「增强鲁棒性」而不挂到具体特征。
- 存在协同关系的 Fk 组合，须有**一段整体论述**：说明单独任一特征都达不到该效果。
...
- **5.1** 每条写「特征描述 + 对应 3.x / Sx / Mx / 式 (n)」，编号与 Step 5 的 Fk **相同**；条目数宜等于站得住的 Fk 数。
