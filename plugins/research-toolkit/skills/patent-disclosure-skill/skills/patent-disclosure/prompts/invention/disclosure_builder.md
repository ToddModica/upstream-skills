...
2. 在 **`## 交付后请确认`** 第 1 条反问 **一次**（请回 **是** / **否**），列出失败公式摘要；说明须额外安装 matplotlib（约 100MB，含 numpy 等），图片在 Word 中不可再编辑。细则 `prompts/delivery_confirm.md`。
...
若 Step 2 / 补材料的 `cad_scan` 为 `ask_enable_step_parse`：**先完成交底落盘**，再在 **`## 交付后请确认` 第 1 条**反问一次是否开启 STEP 多视角解析（文案见 `cad_scan` 的 `messages.ask_enable_step_parse`）。**禁止**在 Step 2 中断成文。未得 **是** 禁止 `bootstrap_cad_venv.py` / `run_step_to_views.py`。用户成文前已主动要求开启的除外。`hint_export_step` 则仅在该节第 1 条提示导出 STEP。细则见 `project_scan.md`「CAD / STEP」。
